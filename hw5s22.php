...
. userids22 > text
. emails22s22 > email field
. entryYears22 > entry year > radio button(s) (use 2016,2017,2018,2019,2020,0 (label = other))
. birthYears22 > use select box (1960 to now)
. gradDates22 > Expected Graduation Date  (date field)
. cses22 > Are you a CSE major> use checkbox
. cars22 > Do you own a car > checkbox
. mobileDevicess22 > Number of mobile devices (including laptops, ipads, etc) you own > select (0-10)
. quotes22 > Favorite quote > text area (5 lines by 40 columns)
. quoteCites22 > Source of quote (site, who said it, etc) >  url
. commands22 > Hidden field with value = "submit"
...
//
	$FIELDS=array( "userids22", "emails22", "entryYears22", "birthYears22", "gradDates22", "mobileDevicess22", "quotes22", "quoteCites22", "commands22");
	$err=0;
...
if ($cmd=="submit") {
	$uid = getVar("userids22");
	$entryYear = getVar("entryYears22");
	$birthYear = getVar("birthYears22");
	$cse = getVar("cses22");
	$car = getVar("cars22");
	$mobileDevices = getVar("mobileDevicess22");
    if (!is_numeric($mobileDevices))
      $mobileDevices = -1;
	$quote = getVar("quotes22");
	$quoteCite = getVar("quoteCites22");
